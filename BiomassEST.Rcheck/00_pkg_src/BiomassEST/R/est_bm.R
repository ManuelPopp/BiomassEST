est.bm <- function(Species, RCD = NA, BHD = NA, H = NA, use = NA, H_meas = NA){
  ...
  if(is.numeric(RCD) & !is.numeric(H) | is.numeric(RCD) & use == "RCD"){
    AGB <- est.bm.RCD(Species = Species, RCD = RCD, H_meas = H_meas)
  }else if(is.numeric(H) & !is.numeric(RCD) | is.numeric(H) & use == "H"){
    AGB <- est.bm.H(Species = Species, H = H)
  }else if(is.numeric(RCD) & is.numeric(H) & is.na(use)){
    AGB <- est.bm.RCD2H(Species = Species, RCD = RCD, H = H, H_meas = H_meas)
  }
  return(AGB)
}
